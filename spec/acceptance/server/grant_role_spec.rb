...
describe 'postgresql::server::grant_role:', :unless => UNSUPPORTED_PLATFORMS.include?(fact('osfamily')) do
  ...
  it 'should grant a role to a superuser' do
    begin
      pp = <<-EOS.unindent
        $db = 'grant_role_test'
        $user = 'psql_grant_role_super_tester'
        $group = 'test_group'
        $password = 'psql_grant_role_pw'
      ...
      ## Check that the role was granted to the user
      psql('--command="SELECT 1 FROM pg_auth_member WHERE roleid=\'test_group\'::regrole AND member=\'psql_grant_role_super_tester\'::regrole', 'psql_grant_role_tester') do |r|
        expect(r.stdout).to match(/\(1 row\)/)
        expect(r.stderr).to eq('')
      end
    end
